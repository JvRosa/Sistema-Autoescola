      ...
      <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
        ...
        if($item->servico > 0){
          ...
          $nome_recep = @$nome_recep->nome;
            ...
            <tr>
            <td><i
              class="fas fa-square mr-1 text-success <?php  if ($item->pago != 'Sim') { ?> text-danger <?php  } ?>"></i>{{$item->descricao}}
            </td>
            <td>R$ {{$valor}}</td>
            ...
            <td>{{$data}}</td>
            <td><a href="{{ URL::asset('img/contas/' . $item->arquivo) }}"
              target="_blank"><?php  if ($item->arquivo != '') { ?><i class="fas fa-eye mr-1 text-info"></i>Arquivo
              <?php  } ?></a></td>
            <td>
              <?php  if ($item->pago != 'Sim') { ?>
              <a title="Baixar Pagamento" href="{{route('pagar.modal-baixa', $item)}}"><i
                class="fas fa-coins text-success mr-1"></i></a>
              <a title="Excluir Dados" href="{{route('pagar.modal', $item)}}"><i
...
<div class="modal fade" id="exampleModal2" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Dar Baixa na Cobrança?</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
